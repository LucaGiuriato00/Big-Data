...
def exactOutliers(X_in, D, M, K):
    ...
    N = len(X)
    outliers = []
    distances = np.zeros((N,N),dtype='float16')
    counter = 0
    for i in range(0, N):
        point = [X[i][0], X[i][1]]
        ...
        for j in range(i+1, N):
            ...
            # I calculate the distance between each point: I need N*(N-1)/2 computations. This is because distances[i,j]=distances[j,i]
    ...
    # Now I have all the distances (saved in a table), calculated with complexity N(N-1)/2: I can compute the check for the number of outliers.
    counter_col = np.zeros(N)
    overall_distance = np.zeros(N, dtype='float16')
    for i in range(N):
        for j in range(i+1,N):
    ...
    # print("COUNTED:", counter_col)
    outliers = sorted(outliers, key=lambda x: x[2], reverse=False)
...
def main():
    # COMMAND LINE CHECK
    # Checking the number of parameters passed through the command line: we need 5 parameters in total.
